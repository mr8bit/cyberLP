...
from page_editor.settings import FOLDER_REGEX
...
class CreateDirForm(forms.Form):
    ...
    name = forms.CharField(widget=forms.TextInput(attrs=dict({'class': 'vTextField'}, max_length=50, min_length=3)),
                           label=_(u'Name'),
    ...
    def __init__(self, path, *args, **kwargs):
        self.path = path
        self.site = kwargs.pop("page_editor_site", None)
...
class ChangeForm(forms.Form):
    ...
    name = forms.CharField(widget=forms.TextInput(attrs=dict({'class': 'vTextField'}, max_length=50, min_length=3)),
                           ...
                           help_text=_(u'Only letters, numbers, underscores, spaces and hyphens are allowed.'),
                           required=True)
    text = forms.CharField(widget=forms.Textarea(attrs=dict({'class': 'html-editor'}, min_length=3)), label=_(u'Text'),
                           required=True)
    ...
    def clean_name(self):
        ...
        if self.cleaned_data['name']:
            ...
            if self.site.storage.isdir(
                    os.path.join(self.path, convert_filename(self.cleaned_data['name']))) and os.path.join(self.path,
                                                                                                           convert_filename(
                                                                                                               self.cleaned_data[
                                                                                                                   'name'])) != self.fileobject.path:
                raise forms.ValidationError(_(u'The Folder already exists.'))
            elif self.site.storage.isfile(
                    os.path.join(self.path, convert_filename(self.cleaned_data['name']))) and os.path.join(self.path,
                                                                                                           convert_filename(
                                                                                                               self.cleaned_data[
                                                                                                                   'name'])) != self.fileobject.path:
