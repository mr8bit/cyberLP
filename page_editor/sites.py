...
from page_editor import signals
from page_editor.base import FileListing, FileObject
from page_editor.decorators import path_exists, file_exists
from page_editor.storage import FileSystemStorageMixin
from page_editor.templatetags.fb_tags import query_helper
from page_editor.utils import convert_filename
from page_editor.settings import (DIRECTORY, EXTENSIONS, SELECT_FORMATS, ADMIN_VERSIONS, ADMIN_THUMBNAIL, MAX_UPLOAD_SIZE,
                       NORMALIZE_FILENAME,
                       CONVERT_FILENAME, SEARCH_TRAVERSE, EXCLUDE, VERSIONS, VERSIONS_BASEDIR, EXTENSION_LIST,
                       DEFAULT_SORTING_BY, DEFAULT_SORTING_ORDER,
                       LIST_PER_PAGE, OVERWRITE_EXISTING, DEFAULT_PERMISSIONS, UPLOAD_TEMPDIR, PATH_EDIT)
...
def get_site_dict(app_name='page_editor'):
    """
...
def get_default_site(app_name='page_editor'):
    """
...
def get_filterdate(filter_date, date_time):
    ...
    if filter_date == 'today' and int(date_year) == int(localtime()[0]) and int(date_month) == int(
            localtime()[1]) and int(date_day) == int(localtime()[2]):
        returnvalue = 'true'
...
class FileBrowserSite(object):
    ...
    def __init__(self, name=None, app_name='page_editor', storage=default_storage):
        self.name = name
    ...
    def get_urls(self):
        ...
        urlpatterns = [
            ...
            url(r'^delete_confirm/$', file_exists(self, path_exists(self, filebrowser_view(self.delete_confirm))),
            ...
            url(r'^create_file/$', file_exists(self, path_exists(self, filebrowser_view(self.create_file))),
                name="fb_create_file"),
    ...
    def browse(self, request):
        ...
        return TemplateResponse(request, 'page_editor/index.html', dict(
    ...
    def createdir(self, request):
        "Create Directory"
        from page_editor.forms import CreateDirForm
        query = request.GET
        path = u'%s' % os.path.join(self.directory, query.get('dir', ''))
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    signals.filebrowser_pre_createdir.send(sender=request, path=server_path,
                                                           name=form.cleaned_data['name'], site=self)
                    self.storage.makedirs(server_path)
                    signals.filebrowser_post_createdir.send(sender=request, path=server_path,
                                                            name=form.cleaned_data['name'], site=self)
        ...
        request.current_app = self.name
        return TemplateResponse(request, 'page_editor/createdir.html', dict(
            admin_site.each_context(request),
            **{
                'form': form,
                'query': query,
    ...
    def upload(self, request):
        ...
        request.current_app = self.name
        return TemplateResponse(request, 'page_editor/upload.html', dict(
            admin_site.each_context(request),
            **{
                'query': query,
    ...
    def delete_confirm(self, request):
        ...
        request.current_app = self.name
        return TemplateResponse(request, 'page_editor/delete_confirm.html', dict(
            admin_site.each_context(request),
            **{
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if "_continue" in request.POST:
                    redirect_url = reverse("page_editor:fb_detail", current_app=self.name) + query_helper(query,
                                                                                                          "filename=" + new_name,
                                                                                                          "filename")
                else:
                    redirect_url = reverse("page_editor:fb_browse", current_app=self.name) + query_helper(query, "",
            ...
            return TemplateResponse(request, 'page_editor/create_file.html', dict(
                admin_site.each_context(request),
                **{
                    'form': form,
                    'fileobject': '',
    ...
    def detail(self, request):
        ...
        fileobject = FileObject(os.path.join(path, query.get('filename', '')), site=self)
        file_read = None
        if fileobject.filetype == 'JS Script' or fileobject.filetype == 'CSS' or fileobject.filetype == 'Template':
            path_file_open = str(settings.PATH_TEMPLATES) + '/' + str(fileobject.path)
            F = open(path_file_open, 'r')
            file_read = F.read()
            F.close()
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    if action_name:
                        ...
                        # Pre-action signal
                        signals.filebrowser_actions_pre_apply.send(sender=request, action_name=action_name,
                                                                   fileobject=[fileobject], site=self)
                        # Call the action to action
                        action_response = action(request=request, fileobjects=[fileobject])
                        # Post-action signal
                        signals.filebrowser_actions_post_apply.send(sender=request, action_name=action_name,
                                                                    fileobject=[fileobject], result=action_response,
                                                                    site=self)
                    if new_name != fileobject.filename:
                        signals.filebrowser_pre_rename.send(sender=request, path=fileobject.path,
                                                            name=fileobject.filename, new_name=new_name, site=self)
                        fileobject.delete_versions()
                        self.storage.move(fileobject.path, os.path.join(fileobject.head, new_name))
                        signals.filebrowser_post_rename.send(sender=request, path=fileobject.path,
                                                             name=fileobject.filename, new_name=new_name, site=self)
            ...
            form = ChangeForm(initial={"name": fileobject.filename, "text": file_read}, path=path,
                              fileobject=fileobject, filebrowser_site=self)

        request.current_app = self.name
        return TemplateResponse(request, 'page_editor/detail.html', dict(
            admin_site.each_context(request),
            **{
                'form': form,
                'fileobject': fileobject,
    ...
    def version(self, request):
        ...
        request.current_app = self.name
        return TemplateResponse(request, 'page_editor/version.html', dict(
            admin_site.each_context(request),
            **{
                'fileobject': fileobject,
                'query': query,
    ...
    def _upload_file(self, request):
        ...
        if request.method == "POST":
            ...
            fb_uploadurl_re = re.compile(r'^.*(%s)' % reverse("page_editor:fb_upload", current_app=self.name))
...
storage = FileSystemStorage(location=PATH_EDIT,base_url='/static')
# Default FileBrowser site
site = FileBrowserSite(name='page_editor', storage=storage)

# Default actions
from page_editor.actions import flip_horizontal, flip_vertical, rotate_90_clockwise, rotate_90_counterclockwise, \
    rotate_180
